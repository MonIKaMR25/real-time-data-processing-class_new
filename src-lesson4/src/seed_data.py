...
Generation runs *inside DuckDB* and streams straight into Postgres via the
postgres extension's server-side COPY — same trick as Lesson 3's loader. This
seeds 1M rows in seconds instead of minutes of python executemany().

Usage:
    python src/seed_data.py                       # 1M orders, 50k customers, 90 days
    python src/seed_data.py --orders 5000000 --days 30
"""
...
def seed(n_orders: int, n_customers: int, n_days: int) -> None:
    ...
    region_case = " ".join(
        f"WHEN '{c}' THEN '{REGIONS[c]}'" for c in CITIES
    )
    ...
    con.sql(f"""
        INSERT INTO pg.orders (customer_id, amount, status, created_at)
        SELECT
